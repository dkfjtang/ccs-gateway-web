...
## CCS Gateway Web fork notes

This fork tracks the official CC Switch v3.16.2 baseline for core provider presets, routing behavior, release notes, and user-facing documentation. Fork-specific patches are kept as explicit overlays:

- Web/WSL runtime support for the managed `ccs-gateway-web` service, including container-oriented local publishing scripts and sanitized operational runbooks.
- Token Saver / TokenFilterEngine cost-control support for tool-output compression. It is off by default and does not rewrite final assistant responses, SSE protocol framing, or usage parsing semantics.
- Responses provider stickiness for long Codex Responses sessions, preventing provider-bound encrypted state from crossing provider boundaries.
- Provider usage probes and billing-rate display scaffolding, kept as an explicit extension point for future routing decisions such as price-aware switching.
- Caveman prompt profiles as prompt-level style/cost presets, without proxy response rewriting.

Public documentation in this fork intentionally uses placeholders for local paths, loopback ports, container IDs, image tags, hostnames, credentials, and runtime evidence.
