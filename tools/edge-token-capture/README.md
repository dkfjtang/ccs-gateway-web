...
7. 重点查看输出里的 `selectedAuthToken`：用 `source`、`length`、`prefix16`、`suffix12` 和 `jwt.expiresAt` 对照浏览器 Network 面板里真实请求的 `Authorization: Bearer ...`，确认扩展选中的 `{{authToken}}` 是否就是你以前手工复制的那个值。
