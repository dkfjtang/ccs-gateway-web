...
1. Codex 接管时，本地配置会被写成 `<proxy-url>/v1`，并强制保持 `wire_api = "responses"`。
...
1. 打开 `路由总开关`，启动本地服务。默认地址是 `<proxy-host>:<proxy-port>`。
...
通常是没有开启 Codex 接管，或者你手动把上游 Chat base URL 直接写给了 Codex。检查 `~/.codex/config.toml` 是否指向 `<proxy-url>/v1`。
