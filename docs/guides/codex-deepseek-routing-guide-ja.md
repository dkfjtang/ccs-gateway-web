...
1. Codex ルーティングを有効にすると、ローカル設定は `<proxy-url>/v1` に書き換えられ、`wire_api = "responses"` は維持されます。
...
1. `ルーティング総スイッチ` をオンにしてローカルサービスを起動します。デフォルトアドレスは `<proxy-host>:<proxy-port>` です。
...
多くの場合、Codex ルーティングが有効になっていないか、上流 Chat base URL を手動で Codex に直接書いています。`~/.codex/config.toml` が `<proxy-url>/v1` を指しているか確認してください。
