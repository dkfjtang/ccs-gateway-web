...
### CCS Gateway Web Fork 说明

本 fork 以官方 v3.16.2 的供应商、路由和文档为基线，只叠加已确认保留的 CCS 扩展：

- 面向托管 `ccs-gateway-web` 服务的 Web runtime 与容器化本地发布流程
- Token Saver / TokenFilterEngine 工具输出压缩，默认关闭，且不改写最终回复或用量解析
- Codex Responses 长会话的供应商粘滞，避免供应商绑定状态跨供应商复用
- 供应商用量探测与计费倍率展示基础能力
- Caveman 提示词配置，作为提示词级成本/风格预设

运维说明统一使用占位符表示本地路径、loopback 端口、镜像标签、容器 ID、主机名和凭据。
