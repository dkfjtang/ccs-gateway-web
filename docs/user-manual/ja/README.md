...
この fork は公式 v3.16.2 のプロバイダー、ルーティング、ドキュメントを基準にし、確認済みの CCS 拡張だけを重ねています：

- 管理対象 `ccs-gateway-web` サービス向けの Web runtime とコンテナ前提のローカル公開フロー
- Token Saver / TokenFilterEngine によるツール出力圧縮（既定では無効、最終応答や使用量解析は書き換えません）
- 長い Codex Responses セッション向けのプロバイダー stickiness
- プロバイダー使用量 probe と課金倍率表示の基盤
- Caveman prompt profiles（プロンプトレベルのコスト / スタイルプリセット）

運用メモでは、ローカルパス、loopback ポート、イメージタグ、コンテナ ID、ホスト名、認証情報をプレースホルダーで表記します。
