...
This fork keeps the official v3.16.2 provider, routing, and documentation baseline, then layers on selected CCS extensions:

- Web runtime support for the managed `ccs-gateway-web` service and container-oriented local publishing workflow
- Token Saver / TokenFilterEngine tool-output compression, disabled by default and scoped away from final responses and usage parsing
- Responses provider stickiness for long Codex Responses sessions
- Provider usage probes and billing-rate display scaffolding
- Caveman prompt profiles as prompt-level cost/style presets

Operational notes use placeholders for local paths, loopback ports, image tags, container IDs, hostnames, and credentials.
