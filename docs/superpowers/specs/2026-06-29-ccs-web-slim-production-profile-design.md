...
第一轮 slim 不改变代理行为，不裁剪 usage pipeline，重点裁剪生产不需要的桌面端、扩展生态和本地个人工作流能力。
...
裁剪项必须以 build profile 或 capability manifest 控制，不能从默认完整版本中直接删除。
...
2. `web-admin-core`：Web 管理面必需的 provider、proxy、usage、backup、auth API。
3. `extension-admin`：skills、MCP、local workspace、local sync、desktop helper、third-party local tool management。
