...
function buildS3SecretPreservationKey(values: {
  region?: string | null;
  bucket?: string | null;
...
export function WebdavSyncSection({
  ...
  const pendingS3SecretPreservationRef = useRef<{
    key: string;
  ...
  useEffect(() => {
    ...
    const nextRegion = s3Config.region ?? "";
    const nextBucket = s3Config.bucket ?? "";
    const nextAccessKeyId = s3Config.accessKeyId ?? "";
    const nextEndpoint = s3Config.endpoint ?? "";
    const nextRemoteRoot = s3Config.remoteRoot ?? "cc-switch-sync";
    const nextProfile = s3Config.profile ?? "default";
    const nextKey = buildS3SecretPreservationKey({
      region: nextRegion,
      bucket: nextBucket,
    ...
    const shouldPreserveRedactedSecret =
      !s3Config.secretAccessKey &&
      pendingS3SecretPreservationRef.current?.key === nextKey &&
      !!pendingS3SecretPreservationRef.current.secretAccessKey;
    setS3Region(nextRegion);
    setS3Bucket(nextBucket);
    setS3AccessKeyId(nextAccessKeyId);
    setS3SecretAccessKey(
      shouldPreserveRedactedSecret
        ? pendingS3SecretPreservationRef.current!.secretAccessKey
        : (s3Config.secretAccessKey ?? ""),
    );
    pendingS3SecretPreservationRef.current = null;
    setS3Endpoint(nextEndpoint);
    setS3RemoteRoot(nextRemoteRoot);
    setS3Profile(nextProfile);
  ...
  const handleS3Save = useCallback(async () => {
    ...
    try {
      ...
      pendingS3SecretPreservationRef.current = s3Settings.secretAccessKey
        ? {
            key: buildS3SecretPreservationKey(s3Settings),
