...
fn s3_key(settings: &S3SyncSettings, artifact: &str) -> String {
    let remote_root = s3_path_segment(&settings.remote_root);
    let profile = s3_path_segment(&settings.profile);
    format!(
        "{}/v{}/db-v{}/{}/{}",
        remote_root, PROTOCOL_VERSION, DB_COMPAT_VERSION, profile, artifact
    )
}
...
fn s3_dir_display(settings: &S3SyncSettings) -> String {
    ...
    format!(
        "{}/v{}/db-v{}/{}",
        remote_root, PROTOCOL_VERSION, DB_COMPAT_VERSION, profile
    )
}

fn s3_path_segment(value: &str) -> String {
    value
        .split('/')
...
mod tests {
    ...
    #[test]
    fn s3_key_normalizes_root_and_profile_slashes() {
        let settings = S3SyncSettings {
            remote_root: "/team//cc-switch-sync/".to_string(),
            profile: "/work//laptop/".to_string(),
            ..S3SyncSettings::default()
        };

        assert_eq!(
            s3_key(&settings, "manifest.json"),
            "team/cc-switch-sync/v2/db-v6/work/laptop/manifest.json"
        );
        assert_eq!(
            s3_dir_display(&settings),
            "team/cc-switch-sync/v2/db-v6/work/laptop"
        );
    }
