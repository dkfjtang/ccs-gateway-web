...
pub struct RequestLogDetail {
    ...
    /// 写入时实际用于计价的模型名。None = v11 前历史行，"" = 未计价的错误行。
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// SQL scalar for the effective priced model: prefer non-empty `pricing_model`,
/// then fall back to `model` for historical rows.
fn effective_model_sql(alias: &str) -> String {
    format!("COALESCE(NULLIF({alias}.pricing_model, ''), {alias}.model)")
}

/// Fold Claude Desktop gateway usage into the Claude dashboard bucket without
/// rewriting stored rows. Request details and limit checks keep the raw app_type.
...
impl Database {
    ...
    pub fn get_model_stats(
        ...
        // UNION detail logs + rollup data. Group by the effective priced model so
        // request-mode routing is attributed to the model that actually priced it.
        let fresh_input_detail = fresh_input_sql("l");
        let fresh_input_rollup = fresh_input_sql("r");
...
impl Database {
    ...
    fn get_log_model_pricing_cached(
        ...
        // v11+ rows store the exact model used for pricing. Backfill must use
        // that value only; otherwise router aliases can be permanently priced
        // against the wrong model once the zero-cost row is updated.
...
mod tests {
    ...
    #[test]
    fn test_matching_proxy_log_requires_opencode_cache_creation_tokens() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        create_legacy_nullable_logs_table(&conn)?;
        conn.execute(
            "INSERT INTO proxy_request_logs (
                request_id, app_type, model, input_tokens, output_tokens,
                cache_read_tokens, cache_creation_tokens, status_code, created_at, data_source
            ) VALUES ('opencode-proxy', 'opencode', 'oc-model', 10, 2, 1, 5, 200, 1000, 'proxy')",
            [],
        )?;

        let opencode_key = DedupKey {
            app_type: "opencode",
            model: "oc-model",
            input_tokens: 10,
            output_tokens: 2,
        ...
        assert!(!has_matching_proxy_usage_log(&conn, &opencode_key)?);

        let codex_key = DedupKey {
            app_type: "codex",
            model: "oc-model",
            input_tokens: 10,
            output_tokens: 2,
        ...
        conn.execute(
            "INSERT INTO proxy_request_logs (
                request_id, app_type, model, input_tokens, output_tokens,
                cache_read_tokens, cache_creation_tokens, status_code, created_at, data_source
            ) VALUES ('codex-proxy', 'codex', 'oc-model', 10, 2, 1, 5, 200, 1000, 'proxy')",
            [],
        )?;
        assert!(has_matching_proxy_usage_log(&conn, &codex_key)?);
    ...
    fn test_provider_and_model_filters_cover_detail_and_rollup() -> Result<(), AppError> {
...

        {
            ...
            conn.execute(
                ...
                ('2026-06-08', 'claude', 'prov-a', 'claude-sonnet-4-6',
                 'claude-sonnet-4-6', 'claude-sonnet-4-6', 5, 5, 500, 50, 0, 0, '5.0', 100),
                ('2026-06-08', 'claude', 'prov-b', 'deepseek-v3',
                 'deepseek-v3', 'deepseek-v3', 7, 7, 700, 70, 0, 0, '7.0', 100)",
                [],
            )?;
        }

        let packy = db.get_usage_summary(None, None, None, Some("Packy"), None)?;
        assert_eq!(packy.total_requests, 7);

        let deepseek = db.get_usage_summary(None, None, None, None, Some("deepseek-v3"))?;
        assert_eq!(deepseek.total_requests, 8);
