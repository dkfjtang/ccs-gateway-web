...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    10 => {
                        log::info!("迁移数据库从 v10 到 v11（usage 计价模型与 rollup 维度）");
    ...
    fn migrate_v10_to_v11(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute_batch(
            r#"
            ALTER TABLE usage_daily_rollups RENAME TO usage_daily_rollups_v10_backup;
            ...
            SELECT
                ...
                provider_id,
                model,
                COALESCE(NULLIF(model, ''), '') AS request_model,
                COALESCE(NULLIF(model, ''), '') AS pricing_model,
                request_count,
                success_count,
                input_tokens,
                output_tokens,
                ...
                total_cost_usd,
                avg_latency_ms
            FROM usage_daily_rollups_v10_backup;
