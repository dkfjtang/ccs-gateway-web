...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::AlreadyRunning => "already_running",
        ProxyError::NotRunning => "not_running",
        ProxyError::BindFailed(_) => "bind_failed",
        ProxyError::StopTimeout => "stop_timeout",
        ProxyError::StopFailed(_) => "stop_failed",
        ...
        ProxyError::NoAvailableProvider => "no_available_provider",
        ProxyError::AllProvidersCircuitOpen => "all_providers_circuit_open",
        ProxyError::NoProvidersConfigured => "no_providers_configured",
        ProxyError::ProviderUnhealthy(_) => "provider_unhealthy",
        ProxyError::MaxRetriesExceeded => "max_retries_exceeded",
...
fn compact_error_message(message: &str, max_chars: usize) -> String {
    ...
    let mut compacted: String = message.chars().take(max_chars).collect();
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    #[test]
    fn codex_proxy_error_json_explains_upstream_413_as_provider_limit() {
        let error = ProxyError::UpstreamError {
            status: 413,
            body: Some(json!({"error": {"message": "nginx 413"}}).to_string()),
        };

        let body = codex_proxy_error_json("MiniMax", "gpt-5.5", "/responses", &error);
        let message = body["error"]["message"].as_str().unwrap_or_default();

        assert!(message.contains("Upstream provider rejected the request with HTTP 413"));
        assert!(message.contains("not a CC Switch limit"));
        assert_eq!(body["error"]["provider"], "MiniMax");
